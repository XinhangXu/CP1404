...
class TravelTracker(App):
    ...
    # the following is about build labels and  textinput
        ...
        #self.save_button = Button(text="Save changes and close")   ###
    ...
    def build_widgets_left_col(self):
        ...
        #self.root.ids.left_layout.add_widget(self.save_button)    ##

        self.add_button.bind(on_release=self.control_add_place)
        #self.save_button.bind(on_release=self.write_csv)         ###
...
# add a new place, error_check requires full filled and priority > 0
    def control_add_place(self, *args):
        ...
        if str(self.name_input.text).strip() == '' or str(self.country_input.text).strip() == '' or str(self.priority_input.text).strip() == '':
            ...
            try:
                ...
                # if the format is correct, add it to list or clean the field
                else:
                    self.place_list.add_place(self.name_input.text, self.country_input.text, int(self.priority_input.text))
                    self.place_list.sort(self.spinner.text)
                    self.clear_fields() #self.place_list.write_csv()
                    self.root.ids.right_layout.clear_widgets()
                    self.build_widgets_right_col()
            except ValueError:
    ...
    def build_widgets_right_col(self):
        ...
        # display on the top side, show how many place visited and unvisited
        ...
        # display on the right side of botton, show the status of selected place
        for place in self.place_list.place_list:
            if place[0].status == "v":
                travel_display_button = Button \
                    (text="'{}' in  {} ({}) visited".format(place[0].name, place[0].country, place[0].priority),id=place[0].name)
                travel_display_button.background_color = [88, 88, 0.85, 0.4]

            else:
                travel_display_button = Button \
                    (text="'{}' in  {} ({}) unvisited".format(place[0].name, place[0].country, place[0].priority), id=place[0].name)
                travel_display_button.background_color = [80, 88, 0.3, 0.3]
    ...
    def visit_Places(self, button):
        ...
        if self.place_list.get_place(button.id).status == 'n':
            ...
            self.root.ids.bottom_layout.text = "Already visited {}".format \
                (str(self.place_list.get_place(button.id).name))

        else:
            # convert visited to unvisited
            self.place_list.get_place(button.id).status = 'n'
            self.root.ids.bottom_layout.text = "Not visit yet:{}".format(str(self.place_list.get_place(button.id).name))
        self.places_sort()
        self.root.ids.right_layout.clear_widgets()
        self.build_widgets_right_col()
